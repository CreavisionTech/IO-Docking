...
> 本文档介绍 IO Dock 的 **Fast-LIVO（方案文档中也写作 Fast-LIO）预设**：由**板卡自主**产生
> 「1 Hz 秒脉冲(PPS) + 多路 10 Hz 触发 + 每秒 GPRMC 时间报文」，供 Fast-LIVO / FAST-LIO、
> 激光雷达、相机或需要多传感器同步的应用使用。
>
> **固件要求：标准固件 ≥ 0.2.0**（含 `TIMING` / `BOOT` / `SEQ SAVE` 命令，本仓库旧的 v0.1.4
> 固件不含这些命令）。0.1.4 固件用户请改用旧的网页方案 `demo/demo_lvi_handle.html`。
...
`demo/demo_lvi_handle.html` 是 0.1.4 时代的网页逐秒调度方案（浏览器发 `PWM TICK`/`PWM SYNC`
并自己算 GPRMC）。固件 ≥0.2.0 后推荐改用 `web/timing.html` + 板端 `TIMING` 任务：
波形调度全部在固件内完成，网页只做配置、授时与观测，更稳、断连不掉线。
